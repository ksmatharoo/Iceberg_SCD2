...
public class SCD2SingleTableTest {
    ...
    public static void main(String[] args) throws NoSuchTableException, ParseException {
        ...
        ////////tests////////
        if (test1) {
            MergeHelper.executeMerge(incomingData, tableName);
        } else {
            Map<String, Column> updated = new HashMap<>();
            String[] columns = incomingData.columns();
            for (int i = 0; i < columns.length; i++) {
                updated.put(columns[i], incomingData.col(columns[i]));
            }

            Map<String, Column> inserted = new HashMap<>();
            String[] columnss = incomingData.columns();
            for (int i = 0; i < columnss.length; i++) {
                inserted.put(columnss[i], incomingData.col(columnss[i]));
            }
            IcebergMergeInto.apply(openIdentifier.toString()).
                    using(incomingData.as("source")).
